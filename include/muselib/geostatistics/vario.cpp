...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
//OLD IMPLEMENTATION
// exp_variog exp_variogram (const vector<double> & values, const vector<double> & coord_x, const vector<double> & coord_y, const vector<double> & coord_z, const int n_points, double max_dist,
//                           const double tol_factor)
// {
//     exp_variog ev;
...
//                 if(md<1)
//                     md=0; // punti coincidenti (h=0) -> condizione non verificabile
//                 else
//                 {
//                     if(md > max_dist)
//                         max_dist = md;
...
//     double lag_spacing = (max_dist/2)/n_lags; // max_dist/2 = variogram coverage
...
//         // Controllo "inutile" poichè ora lavoro su una dist_max/2
//         if(ev.h[i]> max_dist)
//             ev.h[i] = max_dist;
...
//             if(md<1)
//                 md=0; //condizione di punti coincidenti (non li devo considerare)
//             else
//             {
//                 //cout<<"Computed distance: "<< md << " Indici: row/col " << row << "/" << col << std::endl;
//                 for(size_t j = 0; j < ev.h.size(); j++)
...
// //                    if(j == ev.h.size()-1)
// //                    {
// //                        cout<<"Check if the distance " << md << " is in the interval: from "<< min_tol << " to "<< max_tol <<endl;
// //                        if(md >= min_tol && md<=max_tol)
// //                        {
// //                            ev.N[j]+=1; //conto le coppie di punti che rientrano in quella banda di tolleranza
...
//::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
//OLD IMPLEMENTATION
// exp_variog exp_variogram2D (const vector<double> & values, const vector<double> & coord_x, const vector<double> & coord_y, const int n_points, double max_dist,
//                           const double tol_factor)
// {
//     exp_variog ev;
...
//             max_dist = limit_dist;
//         else //if(compute_max == true) //se è sul valore di default, allora calcola il massimo della distanza; altrimenti rimane il valore di input
//         {
//             // 1) Compute distances (max distances stored)
//             for(unsigned int i = 0; i < values.size(); i++)
//             {
...
//         std::cout << "Computing costant lags ... COMPLETED." << std::endl;
...
//        if(l==0)  //NUGGET
//        {
//           clean_ev.N.push_back(ev.N[l]);
//           clean_ev.h.push_back(ev.h[l]);
//           clean_ev.gamma.push_back(ev.gamma[l]);
//        }
//        else
//        {
            ntot = ntot + ev.N[l];
...
/*exp_variog experimental_variogram_with_variable_lag_opt(const vector<double> & values,
                                  const vector<double> & coord_x,
                                  const vector<double> & coord_y,
                                  const vector<double> & coord_z,
                                  const int n_points,
                                  const double &percent)    // significato?? ; percent = settato su 10 (di default)
{
    exp_variog ev;
    ...
    // 1) Compute distances (min and max distances stored)
    for(unsigned int i = 0; i < values.size(); i++)
    {
        for(unsigned int j = i+1; j < values.size(); j++)
        {
    ...
    // 2) Compute experimental variogram with variable lag (raggruppiamo i punti sul variogramma in modo da avere 15 punti (distanza;gamma))
    //      - Compute tolerance
    //      - Define h values (15 default points)
    double tolerance = ((max_dist - min_dist) / (n_points-1))/2;
    std::cout << "Tolerance: " << tolerance << std::endl;

    vector<double> tol(n_points);
    tol[0]=tolerance/4;
    ...
    // Tacche sull'asse x: riferimento al vettore seq della funzione del vario_exp
    ev.h.resize(n_points+1);
    ev.h[0]=0;
    ev.h[n_points]=max_dist;

    for(unsigned int j = 1; j < ev.h.size()-1; j++)
    {
        ev.h[j]=ev.h[j-1]+ tol[j-1] + tol[j];
    ...
    for(unsigned int row = 0; row < values.size(); row++)
    {
        for(unsigned int col = row+1; col < values.size(); col++)
        {
            ...
            for(unsigned int j = 1; j < ev.h.size(); j++)
            {
                std::cout << "INDICE j = " << j << std::endl;
                std::cout << "h(j-1) = " << ev.h.at(j-1) << std::endl;

                if(j == ev.h.size()-1) //se mi trovo sull'ultimo intervallo
                {
                    cout<<"CHECK if distance is in: "<< md << " From "<<(ev.h[j-1]-tol[j-1])<<" to "<<(ev.h.at(ev.h.size()-1))<<endl;
                    if(md >= (ev.h[j-1]-tol[j-1]) && md <= (ev.h.at(ev.h.size()-1)))
                    {
                        ev.N[j-1]+=1;
                        ...
                        cout<<"CHECK on distance: YES. Nh = "<< ev.N.at(j-1) << endl;
                    }
                }
                else
                {
                    cout<<"CHECK if distance is in: "<< md << " From "<<(ev.h[j-1]-tol[j-1])<<" to "<<(ev.h[j-1]+tol[j-1])<<endl;
                    if(md >= (ev.h[j-1]-tol[j-1]) && md < (ev.h[j-1]+tol[j-1]))
                    {
                        ev.N[j-1]+=1;
    ...
    {
        if (ev.N[j-1]>0)
            ev.gamma[j-1] = sum.at(j-1) / ev.N[j-1]; //media punti
        else
            ev.gamma[j-1]=0;

        cout<<ev.N[j-1] <<" ; "<<ev.h[j-1]<<" ; "<<ev.gamma[j-1]<<endl;
    }
...
// Funzione (ottimizzata) per il calcolo del variogramma sperimentale sulla falsa riga della funzione "experimental_variogram_with_variable_lag"
// Questa funzione computa il variogramma sperimentale discretizzato in un numero di punti pari a n_points
// I lag dipendono da una tolleranza pre-calcolata variabile (??funzione per ora NON NOTA!), partendo da una tolleranza iniziale di tol/4 (??)
// Il risultato non considera tutte le coppie di punti che hanno distanza tra MIN_DIST e tol/4
/*exp_variog experimental_variogram_with_variable_lag_opt2(const vector<double> & values,
                                  const vector<double> & coord_x,
                                  const vector<double> & coord_y,
                                  const vector<double> & coord_z,
                                  const int n_points,
                                  const double &percent)    // significato?? ; percent = settato su 10 (di default)
{
    ...
    double start_tol = tolerance/4;
    cout<<"WARNING: start_tolerance is set on tolerance/4: "<< start_tol << std::endl;

    int j = 1;
    vector<double> tol(n_points);
    tol.at(0) = start_tol * (1+percent/100 + j*0.0125);
    cout<<"Computed tol(0): "<< tol[0] << std::endl;
    for(size_t i=1; i<tol.size(); i++)
    {
        j++;
        tol[i] = tol[i-1] * (1+percent/100 + j*0.0125);
        cout<<"Computed tol(i): "<< tol[i] << std::endl;
    }

    // Tacche sull'asse x: riferimento al vettore seq della funzione del vario_exp
    ev.h.resize(n_points);
    ev.h[0] = start_tol + tol[0]; //primo valore di h
    cout<<"Computed h(0): "<< ev.h[0] << std::endl;
    for(size_t i = 1; i < ev.h.size(); i++)
    {
        ev.h[i] = ev.h[i-1] + tol[i-1] + tol[i];
...
// I lag dipendono da una tolleranza pre-calcolata variabile (??funzione per ora NON NOTA!), partendo da una tolleranza iniziale di tol/4 (??)
// Il risultato non considera tutte le coppie di punti che hanno distanza tra MIN_DIST e tol/4
/*exp_variog experimental_variogram_varlag_opt (const vector<double> & values,
                                  const vector<double> & coord_x,
                                  const vector<double> & coord_y,
                                  const vector<double> & coord_z,
                                  const int n_points,
                                  ...
                                  const double &percent)    // significato?? ; percent = settato su 10 (di default)
{
    ...
    for(unsigned int i = 0; i < values.size(); i++)
    {
        for(unsigned int j = i+1; j < values.size(); j++)
        {
            ...
            if(compute_max == true) //se è sul valore di default, allora calcola il massimo della distanza; altrimenti rimane il valore di input
            {
                if(md > max_dist)
                    max_dist = md;
            }
...
//                                  const double &percent)    // significato?? ; percent = settato su 10 (di default)
//{
//    //definire le direzioni e lavorare su ciascuna di esse
//    n_directions = 180/degree_step;

//    vector<double> seq(n_directions);
//    seq[0] = 0; //prima direzione: nord geografico
...
//    //Ciclo sulle direzioni e calcolo il variogramma per ciascuna di esse
//    for(uint dir=0; dir<seq.size(); dir++)
//    {
//        double min_dist = DBL_MAX;
...
////                    double md_x = coord_x[i]-coord_x[j];
////                    double md_y = coord_y[i]-coord_y[j];
...
//        // Tacche sull'asse x: riferimento al vettore seq della funzione del vario_exp
//        dev[dir].h.resize(n_points);
//        dev[dir].h[0] = start_tol + tol[0]; //primo valore di h
//        cout<<"Computed h(0): "<< dev[dir].h[0] << std::endl;
//        for(size_t i = 1; i < dev[dir].h.size(); i++)
//        {
//            dev[dir].h[i] = dev[dir].h[i-1] + tol[i-1] + tol[i];
...
////        dev[dir].h[0] = tolerance * 2; //primo valore di h

////        cout<<"Computed h(0): "<< dev[dir].h[0] << std::endl;
////        for(size_t i = 1; i < dev[dir].h.size(); i++)
////        {
////            dev[dir].h[i] = dev[dir].h[i-1] + 2 * tolerance;
...
//                    double md_x = coord_x[col]-coord_x[row];
//                    double md_y = coord_y[col]-coord_y[row];
////                    double md_x = coord_x[row]-coord_x[col];
////                    double md_y = coord_y[row]-coord_y[col];
...
//        cout<<"################################################"<<endl;
//        cout<<"###### DIRECTIONAL EXPERIMENTAL VARIOGRAM ######"<<endl;
//        cout<<"########### Direction: "<<seq[dir]<<" +/- "<<degree_tolerance<<" ############"<<endl;
//        cout<<"################################################"<<endl;
