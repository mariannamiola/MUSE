...
namespace
{
[[noreturn]] void fail_variogram_decode(const std::string &context, const std::string &message)
{
    std::cerr << "ERROR in variogram decoding [" << context << "]: " << message << std::endl;
    std::exit(1);
}

double resolve_partial_sill_and_model(const MUSE::variogram_methods &fit,
                                      const std::string &context,
                                      std::string &resolved_type,
                                      double &resolved_range)
{
    if (fit.nugget < 0.0)
        fail_variogram_decode(context, "nugget is negative");

    const std::vector<variogram_structure> components = fit.getNestedStructuresAsVariogram();
    if (components.empty())
        fail_variogram_decode(context, "nested_structures is missing or empty");

    double total_partial_sill = 0.0;
    size_t dominant_idx = 0;

    for (size_t i = 0; i < components.size(); ++i)
    {
        const variogram_structure &component = components.at(i);
        const double component_range = std::max(component.range_max, component.range);
        const double component_range_min = (component.range_min > 0.0) ? component.range_min : component_range;

        if (component.type.empty())
            fail_variogram_decode(context, "nested structure with empty model type");
        if (!std::isfinite(component.sill) || component.sill < 0.0)
            fail_variogram_decode(context, "nested structure with invalid sill");
        if (!std::isfinite(component_range) || component_range <= 0.0)
            fail_variogram_decode(context, "nested structure with non-positive range");
        if (!std::isfinite(component_range_min) || component_range_min <= 0.0)
            fail_variogram_decode(context, "nested structure with non-positive minimum range");
        if (component_range_min > component_range)
            fail_variogram_decode(context, "nested structure with range_min > range_max");

        total_partial_sill += component.sill;

        if (components.at(i).sill > components.at(dominant_idx).sill)
            dominant_idx = i;
    }

    if (total_partial_sill <= 0.0)
        fail_variogram_decode(context, "sum of nested partial sills is not positive");

    if ((fit.nugget + total_partial_sill) > 1.000001)
        fail_variogram_decode(context, "nugget + nested partial sills exceeds 1");

    const variogram_structure &dominant = components.at(dominant_idx);
    resolved_type = dominant.type;
    resolved_range = std::max(dominant.range_max, dominant.range);

    if (!std::isfinite(resolved_range) || resolved_range <= 0.0)
        fail_variogram_decode(context, "dominant nested structure has non-positive range");

    return total_partial_sill;
}

// Build the multi-structure (nested) representation on the legacy Variogram so
// that downstream covariance() and parallel_sgs2() evaluate every structure.
// Each component keeps its own type and partial sill; the major/minor ranges
// are anchored to the anisotropy ellipsoid and the per-structure ranges are
// scaled by each component's share of the total fitted range.
void apply_nested_structures_to_variogram(const std::vector<variogram_structure> &components,
                                          const double max_semiaxis,
                                          const double min_semiaxis,
                                          const double azimuth_deg,
                                          Variogram &fvm)
{
    fvm.nested_structures.clear();
    if (components.empty())
        return;

    double total_range = 0.0;
    for (size_t i = 0; i < components.size(); ++i)
        total_range += std::max(components.at(i).range_max, components.at(i).range);

    fvm.nested_structures.reserve(components.size());
    for (size_t i = 0; i < components.size(); ++i)
    {
        const variogram_structure &src = components.at(i);
        const double src_range = std::max(src.range_max, src.range);
        const double fraction = (total_range > 0.0) ? (src_range / total_range)
                                                    : (1.0 / static_cast<double>(components.size()));

        variogram_structure component;
        component.type = src.type;
        component.sill = src.sill;
        component.set_range(fraction * min_semiaxis, fraction * max_semiaxis);
        component.set_azimuth(azimuth_deg);
        fvm.nested_structures.push_back(component);
    }
}

void log_variogram_runtime_model(const std::string &context,
                                 const Variogram &fvm)
{
    if (fvm.nested_structures.empty())
        fail_variogram_decode(context, "runtime nested structures are missing");

    std::cout << "### [" << context << "] Variogram used in simulation: nested with "
              << fvm.nested_structures.size() << " structure(s)." << std::endl;

    double runtime_partial_sill = 0.0;
    for (size_t i = 0; i < fvm.nested_structures.size(); ++i)
    {
        const variogram_structure &runtime_component = fvm.nested_structures.at(i);
        const double runtime_range_max = std::max(runtime_component.range_max, runtime_component.range);
        const double runtime_range_min = (runtime_component.range_min > 0.0) ? runtime_component.range_min : runtime_range_max;
        runtime_partial_sill += runtime_component.sill;

        std::cout << "###   structure " << (i + 1)
                  << ": type=" << runtime_component.type
                  << ", sill=" << runtime_component.sill
                  << ", range_min=" << runtime_range_min
                  << ", range_max=" << runtime_range_max
                  << ", azimuth=" << runtime_component.get_azimuth() << std::endl;
    }

    std::cout << "###   nugget=" << fvm.nugget
              << ", total_partial_sill=" << runtime_partial_sill << std::endl;
    std::cout << "###   search ellipsoid ranges: min=" << fvm.get_minrange()
              << ", max=" << fvm.get_maxrange()
              << ", z=" << fvm.get_zrange() << std::endl;

    std::cout << std::endl;
}

void configure_omni_variogram_from_fit(const MUSE::variogram_methods &fit,
                                       Variogram &fvm,
                                       MUSE::variogram_methods &fitvariov,
                                       const std::string &context)
{
    std::string resolved_type;
    double resolved_range = 0.0;
    const double partial_sill = resolve_partial_sill_and_model(fit, context, resolved_type, resolved_range);

    if (!std::isfinite(resolved_range) || resolved_range <= 0.0)
        fail_variogram_decode(context, "omnidirectional variogram has non-positive range");

    variogram_type type;
    convert_from_str(resolved_type, type);

    fvm.set_range(resolved_range);
    fvm.nugget = fit.nugget;
    fvm.sill = partial_sill;
    fvm.type = type;

    const std::vector<variogram_structure> components = fit.getNestedStructuresAsVariogram();
    apply_nested_structures_to_variogram(components,
                                         resolved_range,
                                         resolved_range,
                                         0.0,
                                         fvm);
    ...
    fitvariov.set_range(resolved_range);
    fitvariov.setType(resolved_type);

    log_variogram_runtime_model(context,
                                fvm);
}

void configure_directional_variogram_from_fit(const MUSE::variogram_methods &fit,
                                              const MUSE::EllipseParameter &summary,
                                              const bool override_z,
                                              const double z_range,
                                              Variogram &fvm,
                                              MUSE::variogram_methods &fitvariov,
                                              const std::string &context)
{
    if (!std::isfinite(summary.min_semiaxis) || !std::isfinite(summary.max_semiaxis) ||
        summary.min_semiaxis <= 0.0 || summary.max_semiaxis <= 0.0)
    {
        fail_variogram_decode(context, "ellipse summary has non-positive semiaxes");
    }
    if (summary.min_semiaxis > summary.max_semiaxis)
        fail_variogram_decode(context, "ellipse summary has min_semiaxis > max_semiaxis");

    if (override_z)
    {
        if (!std::isfinite(z_range) || z_range <= 0.0)
            fail_variogram_decode(context, "z-range override is not positive");
        fvm.set_range(summary.min_semiaxis, summary.max_semiaxis, z_range);
    }
    else
    {
        fvm.set_range(summary.min_semiaxis, summary.max_semiaxis);
    }
    fvm.set_azimuth(summary.max_direction);
    ...
    const std::vector<variogram_structure> components = fit.getNestedStructuresAsVariogram();
    apply_nested_structures_to_variogram(components,
                                         summary.max_semiaxis,
                                         summary.min_semiaxis,
                                         summary.max_direction,
                                         fvm);
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        if (interpolationCompute.isSet() && modeCompute.isSet())
        {
            ...
            if ((get_filename(list_dir.at(0)).compare("data") == 0 && get_filename(list_dir.at(1)).compare("metadata") == 0) || (get_filename(list_dir.at(1)).compare("data") == 0 && get_filename(list_dir.at(0)).compare("metadata") == 0))
            {
                list_dir.clear();
            ...
            for (const std::string &l : list_dir)
            {
                ...
                case varType::NUMBER: // VARIABILE CONTINUA
                {
                    ...
                    else
                    {
                        ...
                        normal_values = normal_score(conv_values,smoothed_cdf,true);
                    }
                    //print a summary of normal score values
                    std::cout << "Normal Score Statistical Summary ..." << std::endl;
                    summary(normal_values.values);
                    ...
                    {
                        configure_omni_variogram_from_fit(metavario.getFitExpVariog(0),
                                                          fvm,
                                                          fitvariov,
                                                          "compute-mode-fixed/OMNI");
                    ...
                    case VarioDirection::DIR:
                    {
                        ...
                        configure_directional_variogram_from_fit(metavario.getFitExpVariog(0),
                                                                 metavario.getSummary(),
                                                                 setZRange.isSet(),
                                                                 setZRange.getValue(),
                                                                 fvm,
                                                                 fitvariov,
                                                                 "compute-mode-fixed/DIR");
                        ...
                        std::cout << "Type is set on: " << fitvariov.getType() << std::endl;
                        std::cout << std::endl;
            ...
            for (const std::string &l : list_dir)
            {
                ...
                case varType::CATEGORIC:
                {
                    ...
                    for (uint c = 0; c < categ.size(); c++)
                    {
                        ...
                        case VarioDirection::OMNI:
                        {
                            configure_omni_variogram_from_fit(metavario_cat.getFitExpVariog(0),
                                                              fvm_cat,
                                                              fitvariov,
                                                              "compute-auto/categorical/OMNI");
                        ...
                        case VarioDirection::DIR:
                        {
                            ...
                            configure_directional_variogram_from_fit(metavario_cat.getFitExpVariog(0),
                                                                     metavario_cat.getSummary(),
                                                                     setZRange.isSet(),
                                                                     setZRange.getValue(),
                                                                     fvm_cat,
                                                                     fitvariov,
                                                                     "compute-auto/categorical/DIR");
