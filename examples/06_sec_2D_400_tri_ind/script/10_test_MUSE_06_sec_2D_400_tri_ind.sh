...
then
	...
	export DATA_SOURCE=${MUSE}/examples/06_sec_2D_400_tri_ind/data
fi
...
then
	...
	export PROJ=06_sec_2D_400_tri_ind
fi
...
then
	...
	export DATA=amga_class_v_400.csv
fi
...
if [[ $DIR == 'DIR'* ]]; then
  export OUTCOM=${OUTWP}/compute/_dir
else
  export OUTCOM=${OUTWP}/compute/_omni
fi
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR0} --sub ${GEOM} --vario MODEL
muse_vario -V -p ${WP} -v ${VAR0} --sub ${GEOM} --vario MODEL --dir ${DIR} --dim ${DIM} --dirs 0,70,80,90,110 --degtol 15

#compute:
##########  COMPUTE  ###########
muse_compute -C -p ${WP} -v ${VAR0} --sub ${GEOM} --dir ${DIR} --dim ${DIM} -m ${OUTSURF}/${GEOM}.obj --crit IK
