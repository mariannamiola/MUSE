...
then
	...
	export DATA_SOURCE=${MUSE}/examples/08_anti_3D_4000_hex/data
fi
...
then
	...
	export PROJ=08_anti_3D_4000_hex
fi
...
muse_geometry -M -p ${WP} --hex -m ${OUTSURF}/anti_0.5-anti_absz.obj --resx ${RESX} --resy ${RESY} --resz ${RESZ} --vtk
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  ...
  muse_compute -S -p ${WP} -v ${VAR} --sub ${SUB} -m ${OUTMAN}/${GMOD}.vtk --dir ${DIR} --dim ${DIM} --csv

  #back:
  muse_compute -B -p ${WP} -v ${VAR} --sub ${SUB} -m ${OUTMAN}/${GMOD}.vtk --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 1 --csv
  #for (( id=0; id<=$NSIM-1; id++ ))
  #do
  #  muse_compute -B -p ${WP} -v ${VAR} --sub ${SUB} -m ${OUTMAN}/${GMOD}.vtk --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 1 -f ${OUTNORMS}/${VAR}_sgs_$id.csv
  #done

  #statsback:
  muse_compute -S -p ${WP} -v ${VAR} --sub ${SUB} -m ${OUTMAN}/${GMOD}.vtk --dir ${DIR} --dim ${DIM} --space VAR --csv
fi
