...
then
	...
	export DATA_SOURCE=${MUSE}/examples/06_sec_2D_400_tri/data
fi
...
then
	...
	export DATA=samples_sec400.csv
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
       ...
       ##########  DATA  ###########
        ...
        muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/${GMOD}.xyz --opt ${OPT} --setz -1.0 --obj
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} --rotaxis X --rotangle 270 -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 1
else
  muse_compute -C -p ${WP} -v ${VAR} --rotaxis X --rotangle 270 -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS}
