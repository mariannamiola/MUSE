...
then
	...
	export DATA_SOURCE=${MUSE}/examples/06_sec_2D_40_tri/data
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
        ...
        cp -R ${DATA_SOURCE}/${GEOM1}.xyz ${INGEOM}
        muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/${GEOM1}.xyz --opt ${OPT} --setz -1.0 --obj
        muse_geometry -L -p ${WP} -m ${OUTSURF}/${GEOM1}.obj --rotaxis X --rotangle -270 --obj
...
#manip:
##########  MANIPULATE  ###########
#muse_manipulate -E -p ${WP} --geom ${OUTSURF}/anti.obj --rotaxis X --rotangle 270


#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --rotaxis X --rotangle 270 --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --nugget 0.2 --dirs 45,70,80,90,100,110 --degtol 45 --type SPHERICAL --vclean 5 --eps 5.0 #--weight #--lagspac CONSTANT --pstep 0.5


#compute:
##########  COMPUTE  ###########
#export OUTCOM=${OUTWP}/compute/${VAR}_${DIR}${DIM}_${GEOM1}
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} --rotaxis X --rotangle 270 -m ${OUTSURF}/${GEOM1}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 1 --octant --scaleradius 1.5 --input 10 --simulated 6
else
  muse_compute -C -p ${WP} -v ${VAR} --rotaxis X --rotangle 270 -m ${OUTSURF}/${GEOM1}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --octant --scaleradius 1.5 --input 10 --simulated 6

  #stats:
  muse_compute -S -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM1}.obj --dir ${DIR} --dim ${DIM} --csv

  #back:
  muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM1}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 1 --csv

  #statsback:
  muse_compute -S -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM1}.obj --dir ${DIR} --dim ${DIM} --space VAR --csv
fi
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  ...
  muse_compute -D -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM1}.obj --dir ${DIR} --dim ${DIM} --space VAR
fi
