...
then
	...
	export DATA_SOURCE=${MUSE}/examples/21_DGM_PrecAgri_tet/data
fi
...
then
	...
	export PROJ=21_DGM_PrecAgri_tet
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
    	...
    	muse_geometry -L -p ${WP} -m ${OUTSURF}/${GEOM}-${GEOM}_dz.obj --scale --sz 100 --obj
    	muse_geometry -M -p ${WP} -m ${OUTSURF}/${GEOM}-${GEOM}_dz_scale.obj --tet --vtk --opt ${OPTVOL}
    	muse_geometry -Z -p ${WP} -m ${OUTVOL}/${GEOM}-${GEOM}_dz_scale.vtk --scale --sz 0.01 --vtk
    	mv ${OUTVOL}/${GEOM}-${GEOM}_dz_scale_scale.vtk ${OUTVOL}/${GEOM}.vtk
    	mv ${OUTVOL}/${GEOM}-${GEOM}_dz_scale_scale.json ${OUTVOL}/${GEOM}.json
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR0} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --zdegtol 5 --eps 5.0 --dirs 70,90,135,170 ##--vclean 10
...
##########  COMPUTE  ###########
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTVOL}/${GEOM}.vtk --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 100 --dir ${DIR} --dim ${DIM} --zrange 0.25
else
  muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTVOL}/${GEOM}.vtk --nsim ${NSIM} --out ${OUTSGS} --dir ${DIR} --dim ${DIM} --zrange 0.25
