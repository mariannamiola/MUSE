...
then
	...
	export DATA_SOURCE=${MUSE}/examples/19_Bisagno_tet/data
fi
...
then
	...
	export DATA=log_samples.csv ##log_samples.csv
fi


# 1. Export variables
#######################################################################
export VEC3D=bisagno_dense
export VEC3DBOUND=Bisagno_boundary_DEM
export DEM=bisagno_DEM_CG
...
#awk '$3 <=1000. {print $0} $3 > 1000. {print $1,$2,0.05}' ${OUTSURF}/${VEC3D}_0@gpkg.dat | sed 's/,/./g'  > ${OUTSURF}/${VEC3D}_0@gpkg.xyz

cp -R ${DATA_SOURCE}/${VEC3DBOUND}.xyz ${INGEOM}

cp -R ${DATA_SOURCE}/${DEM}.xyz ${INGEOM}
muse_geometry -P -p ${WP} --points ${INGEOM}/${DEM}.xyz --tri --obj --boundary ${INGEOM}/${VEC3DBOUND}.xyz #--subset 10000
#muse_geometry -P -p ${WP} --points ${INGEOM}/${DEM}.xyz --tri --obj --boundary ${OUTSURF}/${VEC3D}_0@gpkg.xyz #--subset 10000
...
#muse_manipulate -V -p ${WP} --type SAMPLES --sub ${GEOMNAME} --mgeom ${OUTSURF}/${DEM}.obj --prdir Z
...
muse_vario -V -p ${WP} -v ${VAR} --sub ${GEOMNAME} --vario MODEL --dir ${DIR} --dim ${DIM} --eps 5.0 --lagspac CONSTANT --zdegtol 5 --sttype PROPORTIONAL -f samples_${GEOMNAME}

muse_vario -V -p ${WP} -v ${VAR} --sub ${GEOMNAME} --vario MODEL --dir ${DIR} --dim 3Dz --lagspac CONSTANT --zdegtol 5 --eps 5.0 --sttype PROPORTIONAL -f samples_${GEOMNAME}


#compute:
##########  COMPUTE  ###########
export GEOMNAME=${DEM}-${DEM}_dz
muse_compute -C -p ${WP} -v ${VAR} --dir ${DIR} --dim ${DIM} -m ${OUTMAN}/geom_${GEOMNAME}.vtk --crit SISIM --nsim ${NSIM} --sub ${GEOMNAME} --zrange 15
