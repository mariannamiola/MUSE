...
then
	...
	export DATA_SOURCE=${MUSE}/examples/18_Sicilia/data
fi
...
then
	...
	export DATA=sicily_samples.csv
fi
...
declare -a VAR=("na" "mg" "k" "ca" "al" "fe" "ti" "p" "mn" "ba" "sr" "s" "li" "be" "b" "bi" "sc" "v" "cr" "co" "ni" "cu" "zn" "ga" "ge" "arsenico" "rb" "y" "zr" "nb" "mo" "ag" "indium" "sn" "sb" "te" "cs" "cd" "se" "w" "au" "tl" "pb" "u" "hg" "hf" "ta" "th" "la" "ce" "pr" "nd" "sm" "eu" "gd" "tb" "dy" "ho" "er" "tm" "yb" "lu")
...
for v in "${VAR[@]}"
do
	muse_vario -V -p ${WP} -v ${v} --nscore YES --vario MODEL #--decl --csize ${CELLSIZE} --nstep ${NSTEPDECL} #--vclean 4
done

#variospec:
##muse_vario -V -p ${WP} -v cr --nscore YES --vario MODEL --lagspac CONSTANT --type SPHERICAL #--decl --csize ${CELLSIZE} --nstep ${NSTEPDECL} #--vclean 4
