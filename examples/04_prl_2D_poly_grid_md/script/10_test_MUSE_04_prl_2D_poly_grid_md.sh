...
then
	...
	export DATA_SOURCE=${MUSE}/examples/04_prl_2D_poly_grid_md/data
fi
...
then
	...
	export PROJ=04_prl_2D_poly_grid_md
fi
...
#For geometry
export OPT=Y
export BBP1=1379000,4848000,0
export BBP2=1585800,4848000,0
export BBP3=1585800,4947000,0
export BBP4=1379000,4947000,0
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
        ...
        ### Command for creating rectangular grid: grid.obj
    	muse_geometry -G -p ${WP} --grid --resx 200 --resy 200 --bbp ${BBP1} --bbp ${BBP2} --bbp ${BBP3} --bbp ${BBP4} --obj
    	
    	### Merging trimesh to obtain Liguria trimesh: Liguria_tri.obj
    	muse_geometry -U -p ${WP} -m ${OUTSURF}/${name_geom[0]}_tri.obj -m ${OUTSURF}/${name_geom[1]}_tri.obj --obj
        mv ${OUTSURF}/${name_geom[0]}_tri_${name_geom[1]}_tri.obj ${OUTSURF}/Liguria_tri.obj
        for (( id=2; id<=13; id++ ))
        do
            muse_geometry -U -p ${WP} -m ${OUTSURF}/Liguria_tri.obj -m ${OUTSURF}/${name_geom[${id}]}_tri.obj --obj
            mv ${OUTSURF}/Liguria_tri_${name_geom[${id}]}_tri.obj ${OUTSURF}/Liguria_tri.obj
        done
    	
    	### Extract quadmesh from trimesh of Liguria: grid_Liguria_tri.obj
    	muse_geometry -S -p ${WP} --grid -m ${OUTSURF}/grid.obj --boundary ${OUTSURF}/Liguria_tri.obj --obj ##--clean --obj
    	
    	### Command for extracting grid mesh from rectangular grid, respecting boundaries of triangular meshes
    	for g in "${name_geom[@]}"
    	do
        	muse_geometry -S -p ${WP} --grid -m ${OUTSURF}/grid_Liguria_tri.obj --boundary ${OUTSURF}/${g}_tri.obj --obj
        	mv ${OUTSURF}/grid_Liguria_tri_${g}_tri.obj ${OUTSURF}/${g}_grid.obj
        	mv ${OUTSURF}/grid_Liguria_tri_${g}_tri.json ${OUTSURF}/${g}_grid.json
    	done
    else
        if [[ -d ${OUTMAN} ]]; then
           rm -r ${OUTMAN}
    ...
    ### Extract quadmesh from trimesh of Liguria: grid_Liguria_tri.obj
    muse_geometry -S -p ${WP} --grid -m ${OUTSURF}/grid.obj --boundary ${OUTSURF}/Liguria_tri.obj --clean --obj
...
for g in "${name_geom[@]}"
do
    muse_manipulate -E -p ${WP} --geom ${OUTSURF}/${g}_${ped}.obj
done
...
do
   ...
   declare -a type_geom=("_grid") ### "_grid")
   for j in "${type_geom[@]}"
   do
      echo $i$j
      if [[ $OUTSGS == 'MEAN'* ]]; then
          muse_compute -C -p ${WP} -v ${VAR0} --sub ${name_geom[$i]}_${ped} -m ${OUTSURF}/${name_geom[$i]}${j}.obj --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr ${MIN_VAL[$i]} --maxextr ${MAX_VAL[i]}
      else
          muse_compute -C -p ${WP} -v ${VAR0} --sub ${name_geom[$i]}_${ped} -m ${OUTSURF}/${name_geom[$i]}${j}.obj --nsim ${NSIM} --out ${OUTSGS}
          muse_compute -S -p ${WP} -v ${VAR0} --sub ${name_geom[$i]}_${ped} -m ${OUTSURF}/${name_geom[$i]}${j}.obj --csv
     
          muse_compute -B -p ${WP} -v ${VAR0} --sub ${name_geom[$i]}_${ped} -m ${OUTSURF}/${name_geom[$i]}${j}.obj --csv --extr Extr --minextr ${MIN_VAL[i]} --maxextr ${MAX_VAL[i]}
     
          muse_compute -S -p ${WP} -v ${VAR0} --sub ${name_geom[$i]}_${ped} -m ${OUTSURF}/${name_geom[$i]}${j}.obj --space VAR --csv
      fi
   done
done
