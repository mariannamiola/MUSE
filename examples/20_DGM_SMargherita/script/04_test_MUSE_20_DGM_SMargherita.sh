...
export VAR0=T0 ### variabile nodi reali (5 nodi)
export VAR=T ### variabile estesa (5 nodi reali + 10 nodi aggiuntivi)
...
muse_data -C -p ${WP} --sup 100 ##define sup limit for bounded variable (Hmax = 100%)



#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --nscore YES --vario MODEL --vclean 3 --nugget 0.1 #--eps 5.0 --lagspac CONSTANT #--dir DIR --dim 2D
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GEOM}.obj --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr ${MINEX} --maxextr ${MAXEX} --mode FIXED --vario ${VAR}_${DIR}${DIM}/${VAR}.json
else
  muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GEOM}.obj --nsim ${NSIM} --out ${OUTSGS} --mode FIXED --vario ${VAR}_${DIR}${DIM}/${VAR}.json

  muse_compute -S -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GEOM}.obj --csv

  #back:
  muse_compute -B -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GEOM}.obj --extr Extr --minextr ${MINEX} --maxextr ${MAXEX} --csv --mode FIXED --vario ${VAR}_${DIR}${DIM}/${VAR}.json
