...
then
	...
	export DATA_SOURCE=${MUSE}/examples/02_gslib_2D_tri/data
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
        ...
        muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/${GEOM} --opt ${OPT} --obj
    else
        if [[ -d ${OUTMAN} ]]; then
           rm -r ${OUTMAN}
        fi
...
#muse_vario -V -p ${WP} -v ${VAR0} --nscore YES --dir DIR --dim 2D --vario MODEL --dirs 0,45,80,110,135 --degtol 45 --vclean 10 --eps 5.0 --weight  --nugget 0.18 #--pstep 0.7 --tolfac 2.0 --nstartpoints 11 #--lagspac CONSTANT
#muse_vario -V -p ${WP} -v ${VAR0} --dir ${DIR} --dim ${DIM} --dirs 0,45,80,110,135 --nscore YES --vario MODEL --degtol 25 --eps 5.0 --vclean 10 --weight --maxdist 11 --type SPHERICAL --decl --csize 3 --nstep 10 --nugget 0.25 #--pstep 0.7 --nstartpoints 11 --lagspac CONSTANT
...
if [[ $OUTSGS == 'MEAN'* ]]; then
   muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --bnscore --extr Extr --minextr 0 --maxextr 60 --dir ${DIR} --dim ${DIM} --simulated 12 --input 6 --scaleradius 1
else
   muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --simulated 12 --input 6 --scaleradius 1
