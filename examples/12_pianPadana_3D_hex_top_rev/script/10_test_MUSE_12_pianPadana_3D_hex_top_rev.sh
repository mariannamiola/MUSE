...
then
	...
	export DATA_SOURCE=${MUSE}/examples/12_pianPadana_3D_hex_top_rev/data
fi
...
then
	...
	export PROJ=12_pianPadana_3D_hex_top_rev
fi
...
then
	...
	export DATA=TEMPERATURE_extended.csv
fi

### INSERIRE:
### DATASET CON RIMOZIONE POZZO (revisione dati 20/07/2024)
#export DATA=TEMPERATURE.csv ###no dati nuovi di temperatura superficiale
#export DATA=TEMPERATURE_extended.csv ###CON dati nuovi di temperatura superficiale
...
muse_geometry -M -p ${WP} -m ${OUTSURF}/top-bottom.obj --hex --resx 1000 --resy 1000 --resz 100 --vtk
...
#muse_vario -V -p ${WP} -v ${VAR} --nscore YES --dir ${DIR} --dim ${DIM} --dirs 0,22.5,67.5,90,112.5,135,157.5 --degtol 30 --zdegtol 5 --vario MODEL --eps 5.0 --vclean 5 --weight
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} -m ${OUTVOL}/${GMOD}.vtk --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 5 --maxextr 200 --dir ${DIR} --dim ${DIM} --zrange 3600
else
  muse_compute -C -p ${WP} -v ${VAR} -m ${OUTVOL}/${GMOD}.vtk --nsim ${NSIM} --out ${OUTSGS} --dir ${DIR} --dim ${DIM} --zrange 3600
