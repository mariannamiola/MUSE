...
#do
#  source convert_RISL_data.sh ${j} csv
#done


echo "SPLIT data" del file 
sed '1 d ' ${file_rain}.${ext} | sed 's/"//g' > data.csv

awk 'BEGIN {FS=";"} {print $1,$2}' data.csv | sed 's/ /;/g' > labels.csv
awk 'BEGIN {FS=";"} {print $3,$4}' data.csv | sed 's/ /;/g' > coords.csv
awk 'BEGIN {FS=";"} {print $5}' data.csv > values.csv
...
paste -d ";" labels.csv coords.csv values2.csv | sed 's/; /;/g' > temp.csv 

echo "APPEND all files"
cat ../header.csv temp.csv > ${folder_export}/${file_rain}.${ext}
