...
then
	...
	export DATA_SOURCE=${MUSE}/examples/17_IAVCEI_Sant_Ana/data
fi
...
then
	...
	export PROJ=17_IAVCEI_Sant_Ana
fi
...
then
	...
	export DATA=Sant_Ana_all_format.csv
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
	    ...
	    muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/${GEOM}.xyz --opt ${OPT} --obj
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --deg 22.5 --degtol 22.5 --eps 2.0 --type SPHERICAL ##--vclean 2 --weight
...
if [[ $OUTSGS == 'MEAN'* ]]; then
   muse_compute -C -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --nsim ${NSIM} --bnscore --extr Extr --minextr 20 --maxextr 3800 --dir ${DIR} --dim ${DIM}
else
   ...
   muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 20 --maxextr 3800 --csv
