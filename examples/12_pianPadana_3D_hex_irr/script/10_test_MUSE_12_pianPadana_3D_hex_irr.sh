...
then
	...
	export DATA_SOURCE=${MUSE}/examples/12_pianPadana_3D_hex_irr/data
fi
...
then
	...
	export PROJ=12_pianPadana_3D_hex_irr
fi
...
cp -R ${DATA_SOURCE}/${TOP}.xyz ${INGEOM}
cp -R ${DATA_SOURCE}/${BOT}.xyz ${INGEOM}

muse_geometry -P -p ${WP} --tri --points ${INGEOM}/${TOP}.xyz --convex --obj
muse_geometry -P -p ${WP} --tri --points ${INGEOM}/${BOT}.xyz --convex --obj
#mv ${OUTSURF}/${GEOM}_absz.obj ${OUTSURF}/top.obj 

muse_geometry -T -p ${WP} -m ${OUTSURF}/${TOP}.obj -m ${OUTSURF}/${BOT}.obj --obj

muse_geometry -M -p ${WP} -m ${OUTSURF}/${TOP}-${BOT}.obj --hex --resx 1000 --resy 1000 --resz 1000 --vtk
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  ...
  #do
  #  muse_compute -B -p ${WP} -v ${VAR} -m ${OUTVOL}/${GMOD}.vtk --extr Extr --minextr 5 --maxextr 200 -f ${OUTNORMS}/${VAR}_sgs_$id.csv --dir ${DIR} --dim ${DIM}
  #done
