...
then
	...
	export DATA_SOURCE=${MUSE}/examples/14_sec_3D_4000_hex_ind/data
fi
...
then
	...
	export PROJ=14_sec_3D_4000_hex_ind
fi
...
#vario:
##########  VARIO  ###########
#muse_vario -V -p ${WP} -v ${VAR0} --vario MODEL --vclean 10 --rotaxis X --rotangle 270
...
muse_compute -C -p ${WP} -v ${VAR0} --dir ${DIR} --dim ${DIM} -m ${OUTVOL}/${GRID}.vtk --crit SISIM --nsim ${NSIM} --rotaxis X --rotangle 270 ##--scaleradius 1.5 --input 10 --simulated 6


################################
export OUTCOMP=${OUTWP}/compute/${VAR0}_${DIR}${DIM}_${GRID}
