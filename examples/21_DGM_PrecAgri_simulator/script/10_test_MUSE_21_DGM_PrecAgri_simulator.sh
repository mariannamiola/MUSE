...
then
	...
	export DATA_SOURCE=${MUSE}/examples/21_DGM_PrecAgri_simulator/data
fi
...
then
	...
	export PROJ=21_DGM_PrecAgri_simulator
fi
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --dirs 22.5,45,90,110,135 --degtol 45 --eps 100.0 --vclean 2 #--type EXPONENTIAL


#compute:
##########  COMPUTE  ###########
export OUTCOM=${OUTWP}/compute/${VAR}_${DIR}${DIM}_${GEOM}

export OUTNORMS=${OUTCOM}/_normspace
export OUTVARS=${OUTCOM}/_varspace

if [[ $OUTSGS == 'MEAN'* ]]; then
   muse_compute -C -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --nsim ${NSIM} --bnscore --extr Extr --minextr 0 --maxextr 30 --dir ${DIR} --dim ${DIM}
else
   muse_compute -C -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS}
   muse_compute -S -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --dir ${DIR} --dim ${DIM} --csv
   
   muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 30 --csv
   #for (( id=0; id<$NSIM; id++ ))
   #do
   #  muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GEOM}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 30 -f ${OUTNORMS}/${VAR}_sgs_$id.csv --csv
   #done
