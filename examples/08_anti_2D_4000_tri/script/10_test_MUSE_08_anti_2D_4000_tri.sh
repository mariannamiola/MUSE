...
then
	...
	export DATA_SOURCE=${MUSE}/examples/08_anti_2D_4000_tri/data
fi
...
then
	...
	export PROJ=08_anti_2D_4000_tri
fi
...
#strat:
muse_manipulate -T -p ${WP} --sttype PROPORTIONAL --type SAMPLES --sub ${GEOM} --top ${GEOM}_top --bot ${GEOM}_bot --name ${GEOM} #--rotaxis X --rotangle 270 
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --sttype PROPORTIONAL -f samples_${GEOM} --sub ${GEOM} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --dirs 0,70,80,90,110 --degtol 15 --vclean 10 --eps 5.0 #--nugget 0.2 --weight #--lagspac CONSTANT --pstep 0.5 --tolfac 2.0


#compute:
##########  COMPUTE  ###########
export GMOD=geom_${GEOM}
export OUTCOM=${OUTWP}/compute/${VAR}_${GEOM}_${DIR}${DIM}_${GMOD}
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 1 --octant --scaleradius 1.5 --input 10 --simulated 6
else
  muse_compute -C -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --octant --scaleradius 1.5 --input 10 --simulated 6

  #stats:
  muse_compute -S -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --csv

  #back:
  muse_compute -B -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 1 --csv
  #for (( id=0; id<=$NSIM-1; id++ ))
  #do
  #  muse_compute -B -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 1 -f ${OUTNORMS}/${VAR}_sgs_$id.csv --csv
  #done

  #statsback:
  muse_compute -S -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --space VAR --csv
fi
