...
then
	...
	export DATA_SOURCE=${MUSE}/examples/16_matera_PM10_2D_tri/data
fi
...
then
	...
	export PROJ=16_matera_PM10_2D_tri
fi
...
cp ${DATA_SOURCE}/level1.obj ${OUTSURF}
muse_geometry -L -p ${WP} -m ${OUTSURF}/level1.obj
...
export OUTNORMS0=${OUTCOM0}/_normspace
  ...
  if [[ $OUTSGS == 'MEAN'* ]]; then
    ...
    #back:
    muse_compute -B -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 2 --maxextr 12 --csv
  ...
  if [[ $OUTSGS == 'MEAN'* ]]; then
    ...
    muse_compute -D -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GMOD}.obj --space VAR
  fi
