...
then
	...
	export DATA_SOURCE=${MUSE}/examples/09_realtime/data
fi
...
then
	...
	export DATA=initial_samples.csv
fi
...
#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --deg 22.5 --degtol 22.5 --eps 5.0 --vclean 2
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 6500 --dir ${DIR} --dim ${DIM}
else
  muse_compute -C -p ${WP} -v ${VAR0} -m ${OUTSURF}/${GMOD}.obj --nsim ${NSIM} --out ${OUTSGS} --dir ${DIR} --dim ${DIM}

  #stats:
  muse_compute -S -p ${WP} -v ${VAR} -m ${OUTSURF}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --csv

  #back:
  muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 6500 --csv
  #for (( id=0; id<=$NSIM-1; id++ ))
  #do
  #  muse_compute -B -p ${WP} -v ${VAR} -m ${OUTSURF}/${GMOD}.obj --dir ${DIR} --dim ${DIM} --extr Extr --minextr 0 --maxextr 6500 -f ${OUTNORMS}/${VAR}_sgs_$id.csv --csv
  #done
