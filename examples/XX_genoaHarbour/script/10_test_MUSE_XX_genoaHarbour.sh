...
cp -R ${DATA_SOURCE}/${INPUT}.* ${INGEOM}
muse_geometry -V -p ${WP} --tri --obj

muse_geometry -O -p ${WP} -m ${OUTSURF}/${INPUT}.obj --delta -z ${HALF_RESZ} --obj
mv ${OUTSURF}/${INPUT}_dz.obj ${OUTSURF}/${INPUT}_dzsup.obj

muse_geometry -O -p ${WP} -m ${OUTSURF}/${INPUT}.obj --delta -z -${HALF_RESZ} --obj
mv ${OUTSURF}/${INPUT}_dz.obj ${OUTSURF}/${INPUT}_dzinf.obj

muse_geometry -T -p ${WP} -m ${OUTSURF}/${INPUT}_dzsup.obj -m ${OUTSURF}/${INPUT}_dzinf.obj --obj
mv ${OUTSURF}/${INPUT}_dzsup-${INPUT}_dzinf.obj ${OUTSURF}/${INPUT}_closed.obj

##Hexahedral meshing
muse_geometry -M -p ${WP} -m ${OUTSURF}/${INPUT}_closed.obj --hex --resx ${RESX} --resy ${RESY} --resz ${RESZ} --vtk


#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --nscore YES --vario MODEL --maxdist 400 --type EXPONENTIAL --nugget 0.02 


#compute:
##########  COMPUTE  ###########
muse_compute -C -p ${WP} -v ${VAR} -m ${OUTVOL}/${INPUT}_closed.vtk --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 60 --octant --scaleradius 1.5 --input 4 --simulated 3
