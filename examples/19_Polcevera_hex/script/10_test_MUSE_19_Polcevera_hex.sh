...
then
	...
	export DATA_SOURCE=${MUSE}/examples/19_Polcevera_hex/data
fi
...
then
	...
	export DATA=dataset/log_samples_polcevera.csv ##log_samples.csv
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
        ...
        muse_geometry -P -p ${WP} --points ${INGEOM}/${DEM}.xyz --subset 10000 --tri --obj --boundary ${OUTSURF}/${VEC3D}_0@gpkg.xyz
        
        muse_geometry -O -p ${WP} -m ${OUTSURF}/${DEM}.obj --delta -z ${DELTA} --obj
    	muse_geometry -T -p ${WP} -m ${OUTSURF}/${DEM}.obj -m ${OUTSURF}/${DEM}_dz.obj --obj
    	muse_geometry -M -p ${WP} -m ${OUTSURF}/${DEM}-${DEM}_dz.obj --hex --resx ${RESX} --resy ${RESY} --resz ${RESZ} --vtk
    	muse_geometry -Z -p ${WP} -m ${OUTVOL}/${DEM}-${DEM}_dz.vtk --surf --obj
...
##--geom ${OUTVOL}/${GEOMNAME}.vtk --mgeom ${OUTSURF}/${DEM}.obj --mgeom ${OUTSURF}/${DEM}_dz.obj --prdir Z
...
muse_manipulate -T -p ${WP} --sttype PROPORTIONAL --type VOLUME --top ${GEOMNAME}_top --bot ${GEOMNAME}_bot --geom ${OUTVOL}/${GEOMNAME}.vtk --vtk --name ${DEM} --prdir Z
...
muse_vario -V -p ${WP} -v ${VAR} --sttype PROPORTIONAL -f samples_${GEOMNAME} --sub ${GEOMNAME} --nscore YES --vario MODEL --dir ${DIR} --dim ${DIM} --eps 5.0 --lagspac CONSTANT --zdegtol 5 --itype SPHERICAL!4 --inugget 0.04!4 --itype SPHERICAL!5 --inugget 0.06!5 --itype SPHERICAL!6 --inugget 0.12!6 --imaxdist 2000!5 ##--inugget 0.07!4
muse_vario -V -p ${WP} -v ${VAR} --sttype PROPORTIONAL -f samples_${GEOMNAME} --sub ${GEOMNAME} --nscore YES --vario MODEL --dir ${DIR} --dim 3Dz --lagspac CONSTANT --zdegtol 5 --eps 5.0
