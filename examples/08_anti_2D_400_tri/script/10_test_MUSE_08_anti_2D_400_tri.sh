...
then
	...
	export DATA_SOURCE=${MUSE}/examples/08_anti_2D_400_tri/data
fi
...
then
	...
	export PROJ=08_anti_2D_400_tri
fi
...
#strat:
muse_manipulate -T -p ${WP} --sttype PROPORTIONAL --type SAMPLES --sub ${GEOM} --top ${GEOM}_top --bot ${GEOM}_bot --name anti #--rotaxis X --rotangle 270 
muse_manipulate -T -p ${WP} --sttype PROPORTIONAL --type GEOMETRY --top ${GEOM}_top --bot ${GEOM}_bot --geom ${OUTSURF}/${GEOM}.obj --obj --name ${GEOM}

#vario:
##########  VARIO  ###########
muse_vario -V -p ${WP} -v ${VAR} --sttype PROPORTIONAL -f samples_${GEOM} --sub ${GEOM} --nscore YES --dir ${DIR} --dim ${DIM} --vario MODEL --dirs 0,70,80,90,110 --degtol 15 --pstep 0.5 --vclean 10 --eps 5.0 #--weight #--lagspac CONSTANT
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 1
else
  muse_compute -C -p ${WP} -v ${VAR} --sub ${GEOM} -m ${OUTMAN}/${GMOD}.obj --nsim ${NSIM} --dir ${DIR} --dim ${DIM} --out ${OUTSGS}
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  ...
  muse_compute -D -p ${WP} -v ${VAR} -m ${OUTMAN}/${GMOD}.obj --sub ${GEOM} --dir ${DIR} --dim ${DIM} --space VAR
fi
