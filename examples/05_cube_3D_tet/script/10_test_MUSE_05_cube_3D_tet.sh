...
then
	...
	export DATA_SOURCE=${MUSE}/examples/05_cube_3D_tet/data
fi
...
if [[ -d ${WP} ]]; then
    ...
    if $RMPROJ; then
        ...
        muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/cube.xyz --opt ${OPT} --obj
        muse_geometry -O -p ${WP} -m ${OUTSURF}/cube.obj --abs -z -32 --obj
        muse_geometry -T -p ${WP} -m ${OUTSURF}/cube.obj -m ${OUTSURF}/cube_absz.obj --obj
        
        #muse_geometry -P -p ${WP} --tri --polygon ${INGEOM}/cube.xyz --obj ##--opt ${OPT}
        #muse_geometry -O -p ${WP} -m ${OUTSURF}/cube.obj --abs -z -32 --obj
        #muse_geometry -T -p ${WP} -m ${OUTSURF}/cube.obj -m ${OUTSURF}/cube_absz.obj --obj
        #./../../../external/fTetWild/build/FloatTetwild_bin --input ${OUTSURF}/cube-cube_absz.obj --output ${OUTSURF}/cube-cube_absz.mesh -l 0.0085 -e 0.0001
        #muse_geometry -Z -p ${WP} -m ${OUTSURF}/cube-cube_absz.mesh --vtk
    
        muse_geometry -M -p ${WP} -m ${OUTSURF}/cube-cube_absz.obj --tet --opt ${TETOPT} --vtk
...
if [[ $OUTSGS == 'MEAN'* ]]; then
  muse_compute -C -p ${WP} -v ${VAR} -m ${OUTVOL}/${GMOD}.vtk --nsim ${NSIM} --out ${OUTSGS} --bnscore --extr Extr --minextr 0 --maxextr 1 --dir ${DIR} --dim ${DIM} --zrange ${ZRANGE} --octant
else
  muse_compute -C -p ${WP} -v ${VAR} -m ${OUTVOL}/${GMOD}.vtk --nsim ${NSIM} --out ${OUTSGS} --dir ${DIR} --dim ${DIM} --zrange ${ZRANGE} --octant
