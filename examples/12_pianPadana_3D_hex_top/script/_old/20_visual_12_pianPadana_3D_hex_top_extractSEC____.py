...
###from paraview.simple import ProgrammableGlyph
...
citycsv_file=script_dir+'/../data/Città_Padana_NW.csv'
...
# create a new 'CSV Reader'
citycsv = CSVReader(registrationName='city.csv', FileName=[citycsv_file])
citycsv.DetectNumericColumns = 1
citycsv.UseStringDelimiter = 1
citycsv.HaveHeaders = 1
...
# --- Step 3: Extract Selection ---
# We extract point #0 for simplicity. You can loop or use thresholds if needed.
selectionSource = IDSelectionSource()
selectionSource.FieldType = 'POINT'
selectionSource.IDs = [0]  # (index, unused value)
extractSelection = ExtractSelection(Input=contour1)
extractSelection.Selection = selectionSource
...
for value in isosurfaces:
	...
	threshold1.LowerThreshold = value
	threshold1.UpperThreshold = value
...
# Creare il filtro Calculator per calcolare le etichette
#calculator = Calculator(Input=contour1)
#calculator.ResultArrayName = "ContourValues"
#calculator.Function = '"Field 0"'  # Sostituire con il nome corretto del campo
...
tableToPoints3Display = Show(tableToPoints3, renderView2, 'GeometryRepresentation')
...
tableToPoints3Display.SelectInputVectors = [None, '']
...
# Aggiungere etichette per ogni contorno
#for label in labels:
#    Show(label, renderView2)
...
annotateAttributeData1Display = Show(annotateAttributeData1, renderView2, 'TextSourceRepresentation') 
# trace defaults for the display properties.
annotateAttributeData1Display.WindowLocation = 'Any Location'
annotateAttributeData1Display.Position = [0.8780913893974983, 0.33122807017543854]
annotateAttributeData1Display.Color = [0.0, 0.0, 0.0]
annotateAttributeData1Display.FontSize = 16 
...
SaveScreenshot(directory+'/'+sim_name+'_sec_prova.png', renderView2, FontScaling=True, OverrideColorPalette='WhiteBackground', TransparentBackground=False)
