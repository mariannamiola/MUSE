...
extract_section = sys.argv[1].lower() == 'true'
print(f"extract_section: {extract_section}")

extract_profile = sys.argv[2].lower() == 'true'
print(f"extract_profile: {extract_profile}")
#####
...
for stats_name in stats_name_list:
    ...
    if extract_section:
        ...
        if fix_depth:
            ...
            z_value = -abs(float(sys.argv[3]))  # per renderlo negativo
    ...
    if extract_section:
        ...
        if fix_depth:
            ...
            SaveScreenshot(directory+'/'+sim_name+'_sec'+sys.argv[3]+'.png', renderView2, FontScaling=True, OverrideColorPalette='WhiteBackground', TransparentBackground=False)
