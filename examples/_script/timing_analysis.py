...
STYLE = {
    ...
    "ytick.labelsize":      9,
    "legend.fontsize":      8.5,
    "legend.frameon":       False,       # no box on legends globally
    "lines.linewidth":      1.2,
...
# Nature/Science style palette — muted, print-safe
PALETTE = ["#333333", "#666666", "#999999", "#1a6faf", "#c0392b", "#27ae60", "#8e44ad"]
MARKERS = ["o", "s", "^", "D", "v", "P", "X"]
...
def _error_bars(ax, x, y, yerr, color, marker, label):
    ...
    if np.all(np.isnan(yerr)):
        ...
        ax.errorbar(x, y, yerr=yerr, marker=marker, color=color,
                    ...
                    elinewidth=0.8, linewidth=1.2, markersize=3.5)


def _annotate_config(ax, text: str):
    """Config annotation with clean minimal box."""
...
def _shared_legend(fig, ax, n_items: int, title: str):
    """
    Draw a single horizontal legend centred above the figure.
    Removes individual axes legends.
    """
    ...
    # Remove any per-axis legends that matplotlib may have auto-created
    for axes_obj in fig.axes:
        leg = axes_obj.get_legend()
        if leg is not None:
            leg.remove()
...
# ─────────────────────────────────────────────────────────────────────
# Fig 0 — single run: horizontal bar chart
# ─────────────────────────────────────────────────────────────────────
...
def plot_single_run(timing_path: str, n_cells: int, n_sims: int,
    ...
    colors = [PALETTE[3] if p == phase else "#cccccc" for p in df["phase"]]
    ...
    ax.set_title("Elapsed time — MUSE-compute run summary", pad=10)
    ...
    fig.text(0.5, -0.04, cfg, ha="center", fontsize=8.5, color="#666666",
             style="italic")

    from matplotlib.patches import Patch
    legend_els = [Patch(facecolor=PALETTE[3], label=phase),
                  Patch(facecolor="#cccccc", label="Other phases")]
...
# ─────────────────────────────────────────────────────────────────────
# Fig 1 — elapsed time vs N simulations  (multi-curve: mesh resolution)
# ─────────────────────────────────────────────────────────────────────
...
def plot_time_vs_nsims(df: pd.DataFrame, output_dir: Path,
                       ...
                       dpi: int = 300, fmt: str = "pdf"):
    """
    Elapsed time vs number of simulations.
    ...
    fig.suptitle("SGS Runtime Scalability — Number of Simulations",
                 fontweight="bold", y=1.14)
    _shared_legend(fig, axes[0], len(cells_vals), title="Mesh resolution")
    fig.tight_layout()
...
# ─────────────────────────────────────────────────────────────────────
# Fig 2 — elapsed time vs mesh cells  (multi-curve: n_simulations)
# ─────────────────────────────────────────────────────────────────────
...
def plot_time_vs_ncells(df: pd.DataFrame, output_dir: Path,
                        ...
                        dpi: int = 300, fmt: str = "pdf"):
    """
    Elapsed time vs mesh resolution (number of cells).
    ...
    fig.suptitle("SGS Runtime Scalability — Mesh Resolution",
                 fontweight="bold", y=1.14)
    _shared_legend(fig, axes[0], len(sims_vals), title="No. of simulations")
    fig.tight_layout()
...
# ─────────────────────────────────────────────────────────────────────
# Fig 3 — elapsed time vs n_input  (multi-curve: n_simulations)
# ─────────────────────────────────────────────────────────────────────
...
def plot_time_vs_ninput(df: pd.DataFrame, output_dir: Path,
                        ...
                        dpi: int = 300, fmt: str = "pdf"):
    """
    Elapsed time vs number of conditioning input points.
    One curve per number of simulations.
    ...
    fig.suptitle("SGS Runtime Scalability — Conditioning Data",
                 fontweight="bold", y=1.14)
    _shared_legend(fig, axes[0], len(sims_vals), title="No. of simulations")
    fig.tight_layout()
...
def plot_summary_table(df: pd.DataFrame, output_dir: Path,
    ...
    for row in cell_data:
        fmt_cell.append([
            ...
            f"{row[3]:.3f}",
            f"{row[4]:.4f}" if not np.isnan(row[4]) else "—"
        ])
